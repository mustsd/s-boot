    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="用户账号" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-input placeholder="请输入用户账号" v-decorator="[ 'account', {}]" :readOnly="true"/>
        </a-form-item>

        <a-form-item label="登陆密码" :labelCol="labelCol" :wrapperCol="wrapperCol" hasFeedback >
          <a-input type="password" placeholder="请输入登陆密码" v-decorator="[ 'pwd', validatorRules.pwd]" />
        </a-form-item>
  ...
  export default {
    ...
    methods: {
      show (account) {
        ...
        this.$nextTick(() => {
          this.form.setFieldsValue({account:account});
        });
      },
      close () {
        this.$emit('close');
        ...
        this.form.validateFields((err, values) => {
          if (!err) {
            ...
            changePassword(formData).then((res)=>{
              if(res.success){
                this.$message.success('请牢记新密码：' + res.message);
                this.$emit('ok');
              }else{
